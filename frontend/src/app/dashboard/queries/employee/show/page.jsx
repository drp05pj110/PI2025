...
export default function CustomerDetails() {
  ...
  async function handleEmployeeDelete(employeeId) {
    try {
      ...
      router.push(`/dashboard/queries/employee`)
  ...
  return (
    <main className={Styles.container}>
      <div className={Styles.containerCustomerButtons}>
        <h1>Funcionário</h1>
        <Link href={'/dashboard/registrations/employee'}
        className={Styles.button}> Cadastrar Funcionário</Link>
      </div>
     
      {/* Dados do Funcionário */}
      {employee ? (
        <div className={Styles.containerCustomerDetails}>
          <label className={Styles.row}>Nome: {employee.name}</label>
          <label className={Styles.row}>Função: {employee.position}</label>
        <div className={Styles.containerCustomerButtons}>
          <button className={Styles.addressItem} onClick={()=>handleEmployeeDelete(employee.id)} >
                  Excluir
          </button>
          <Link className={Styles.addressItem}
            href={{pathname: '/dashboard/queries/employee/edit',
